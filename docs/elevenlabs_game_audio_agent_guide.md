...
## 27. As implemented in this repo

The Russian voice set is generated by `scripts/voice/generate.mjs` from
`scripts/voice/lines.ru.json` and lands as Vorbis `.ogg` under `assets/voice/ru/`
with metadata in `assets/voice/ru/manifest.json`. See
[flight-map.md](flight-map.md#voice-hints-spec-8-11) for how the game plays them.

Practical notes that diverged from the defaults above:

- **Model:** shipped on `eleven_multilingual_v2`, not `eleven_v3`. v3's audio
  tags (`[excited]`, `[laughs]`, …) are expressive but v3 (alpha) clones the
  Professional Voice Clones we use less faithfully than v2. **v2 ignores audio
  tags** — emotion comes from per-role `voice_settings` (requests = soft
  "pleading"; thanks/arrivals = "cheerful") and from wording; laughter is written
  into the text ("Ха-ха!" / "Хи-хи!"), not tagged.
- **Voice keying:** lines are keyed by target **country**, not character slot, so
  the spoken destination always matches the current round. One narrator voice for
  arrivals/greeting/celebration; five character voices mapped to countries via the
  `content/world.lua` round columns so each friend keeps one voice.
- **Plan:** ElevenLabs library/"professional" voices need a **paid** plan for API
  use (free tier → HTTP 402). `--draft` swaps in free premade English voices for a
  wrong-accent placeholder; drop it on a paid plan.
- **Key:** read from `.env` (`elevenlabs_key`) only; never logged, never called
  from the game. mp3 masters go to `voice-src/` (gitignored); the `.ogg` are shipped.
